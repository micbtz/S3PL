...
// [[Rcpp::export]]
arma::mat grad_i_IRTlikRcppA(arma::vec par, arma::mat data, bool pen, 
                          arma::vec nodes, arma::vec weights, double lambda=0)
{
  int n = data.n_rows;
  int nitems = par.size()/3;
  ...
  for (int k=0;k<nitems;k++) {
    ...
    arma::mat prod_k2 = prod_k % ((2*data.col(k)-1.0) * dPdbeta2.row(k));
    arma::mat prod_k3 = prod_k % ((2*data.col(k)-1.0) * dPdbeta3.row(k));
    out.col(k) = prod_k3 * weights / sumq;
    out.col(nitems+k) = prod_k1 * weights / sumq;
    out.col(2*nitems+k) = prod_k2 * weights / sumq;
  }
  
  if (pen) { 
    arma::mat grpenalty(nitems*3,1);
    grpenalty.zeros();
    arma::mat D(nitems,nitems);
    D.fill(-1);
    D.diag().fill((double)nitems - 1.0);
    grpenalty.rows(0,nitems-1) = -lambda*2*D*beta3;
    //arma::uvec ind = arma::regspace<arma::uvec>(0, nitems - 1);
    out.each_row() += grpenalty.t()/(double)n;
  }
  return out;
}
