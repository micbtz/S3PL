...
tpl <- function(data, penalized = FALSE, BR = FALSE, lambda = 0, cv = FALSE, K = 5, nq = 21, empirical = TRUE,
                M = 5000, ncores = NULL, trace = TRUE, init_BR = FALSE)
{
  ...
  #if (trace) message("=============================================\n")
  res_MLE <- fit3pl_MLE(data = data, nq = nq)
  ...
  # ridge-type penalty
  {
    ...
    #if (trace) message("=============================================\n")
    res_ridgepen <- fit3pl_ridgepen(est_MLE = est_MLE, lambda = lambda, data = data, nq = nq, K = K, trace = trace, cv = cv)
    est_pen <- res_ridgepen$est_pen
    cv_res <- res_ridgepen$cv_res
    conv_pen <- res_ridgepen$conv_pen
    if (cv) est_pen_sel <- est_pen[, cv_res$sel + 1]
  }
  ...
  out <- list(est_MLE = est_MLE, logLik = lik, conv_MLE = conv_MLE,
            est_pen = est_pen,conv_pen = conv_pen, est_pen_sel = est_pen_sel,
            cv_est = cv_res$est, cv_lik = cv_res$lik, lambda_sel = lambda[cv_res$sel],
...
fit3pl_MLE <- function(data, nq) {
  ...
  return(list(est_MLE = est_MLE, lik = lik, conv_MLE = conv_MLE))
}


fit3pl_ridgepen <- function(est_MLE, lambda, data, nq, K, trace, cv)
{
...
print.tpl <- function(x, IRTparam = TRUE,...)
{
  ...
  {
    if (length(x$lambda) > 1 & is.null(x$cv_est)) 
      message("Cross-validation was not run, cannot print the estimates")
    else
    {
      ...
      else est_pen_sel<-matrix(x$est_pen_sel,ncol=3)
...
cv_tpl_ridge<-function(data, lambda, K, est_MLE, gq, trace)
{
  ...
  return(list(est = est, lik = lik, sel = sel-1))
}
